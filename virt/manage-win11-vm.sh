...
cmd_define() {
    ...
    echo "  Primary (Windows):  $NVME_DEV → vdb (VirtIO, iothread 1)"
    echo ""
    echo "Performance features enabled:"
    echo "  - Hugepages, iothreads, host-passthrough CPU w/ cache passthrough"
    echo "  - VirtIO disks with cache=none, io=native, discard=unmap"
    echo "  - Extended Hyper-V enlightenments (ipi, reset, reenlightenment)"
    echo "  - Memory balloon disabled for lower overhead"
    echo ""
    echo "Next steps:"
    ...
    echo "  3. Ensure hugepages are allocated: echo 8192 | sudo tee /proc/sys/vm/nr_hugepages"
    echo "  4. Start the VM:  virsh start $VM_NAME"
    echo "  5. Open viewer:   virt-manager (or virt-viewer $VM_NAME)"
    echo ""
    echo "  NOTE: VirtIO drivers must be installed in Windows for the disks to be visible."
    echo "  If the VM won't boot, switch disk buses back to 'sata' in win11-vm.xml."
}
